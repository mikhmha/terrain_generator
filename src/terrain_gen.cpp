...
{
	auto c = CSurface(glm::vec3(-1.0, 0.0, 1.0), glm::vec3(1.0, 0.0, 1.0), glm::vec3(1.0, 0.0, -1.0), glm::vec3(-1.0, 0.0, -1.0));			// XZ PLANE WITH NORMAL (0.0, 1.0, 0.0)
...
std::vector<BezierSurface> gen_TerrainMasks(unsigned int l, unsigned int w, std::int32_t seed, std::int32_t octaves, float freq)
{
	...
	for (auto y = 0; y != l * 2; y += 2)
	{
		...
		{
			auto m = gen_BezierSurfaceMask(0.0, 0.0);														// gen basic mask with height of all control vertices set to 0.0, if using only 1 height per mask/bezier surface, you can just call this function and pass the height to it
			m[1][1].z = perlin.accumulatedOctaveNoise2D_0_1(x / fx, y / fy, octaves);
			m[1][2].z = perlin.accumulatedOctaveNoise2D_0_1((x + 1.0) / fx, y / fy, octaves);
			m[2][1].z = perlin.accumulatedOctaveNoise2D_0_1(x / fx, (y + 1.0) / fy, octaves);
			m[2][2].z = perlin.accumulatedOctaveNoise2D_0_1((x + 1.0) / fx, (y + 1.0) / fy, octaves);
